	...
	return (
		...
		>
			{meteors.map((_, idx) => {
				...
				const spread = 800; // px total spread
				const position = Math.round(idx * (spread / meteorCount) - spread / 2); // centered

				// Deterministic timings derived from the index
				const maxDelay = 5; // seconds
				const delay = ((idx % 10) / 10) * maxDelay + 0.2; // 0.2 .. ~5s
				const durationBase = 5; // seconds
				const duration = durationBase + (idx % 6); // 5..10s
				...
				return (
					<span
						...
						style={{
							top: "-5px",
							left: `${position}px`,
							animationDelay: `${delay.toFixed(3)}s`, // deterministic
